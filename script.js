...
async function getPublicIp() {
  ...
  const response = await fetch("https://geolocation-db.com/json/", {
    ...
    console.log("City not found in our database of geolocation-db");
	   
}
}

getPublicIp();
// const myKey = 'RRHBUPPUZSJ8H3NHWMQ5ZPHTF' // my key
const myKey ='EJ6UBL2JEQGYB3AA4ENASN62J'  // his key(change after it's ready)
...
async function getWeatherData(city, unit, hourlyorWeek) {
    try {
      ...
      if (unit === "c") {
        ...
        temp.textContent = celciusToFahrenheit(today.temp);
      }
      currentLocation.textContent = data.resolvedAddress;
      condition.textContent = today.conditions;
      rain.textContent = "Precipitation - " + today.precip + "%";
      uvIndex.textContent = today.uvindex;
      windSpeed.textContent = today.windspeed;
      measureUvIndex(today.uvindex);
      mainIcon.src = getIcon(today.icon);
      changeBackground(today.icon);
      humidity.textContent = today.humidity + "%";
      updateHumidityStatus(today.humidity);
      visibilty.textContent = today.visibility;
      updateVisibiltyStatus(today.visibility);
      airQuality.textContent = today.winddir;
...
function updateVisibiltyStatus(visibility) {
  if (visibility <= 0.03) {
    visibilityStatus.textContent = "Dense Fog";
  } else if (visibility <= 0.16) {
    visibilityStatus.textContent = "Moderate Fog";
  } else if (visibility <= 0.35) {
    visibilityStatus.textContent = "Light Fog";
  } else if (visibility <= 1.13) {
    visibilityStatus.textContent = "Very Light Fog";
  } else if (visibility <= 2.16) {
    visibilityStatus.textContent = "Light Mist";
  } else if (visibility <= 5.4) {
    visibilityStatus.textContent = "Very Light Mist";
  } else if (visibility <= 10.8) {
    visibilityStatus.textContent = "Clear Air";
  } else {
    visibilityStatus.textContent = "Very Clear Air";
  }
}
...
function changeUnit(unit) {
  if (currentUnit !== unit) {
    ...
    tempUnit.forEach((elem) => {
      elem.textContent = `°${unit.toUpperCase()}`;
    });
    if (unit === "c") {
